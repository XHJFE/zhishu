...
function is(type, obj) {
    type = type.split('').map(function (v, i) {
        if (i === 0) {
 ...
 * @param typeMap 类型映射表
 */
function resetDataByMap(data, typeMap) {
    let map = {"array": [], "object": {}};
    if (is('Object', typeMap)) {
        for (var k in typeMap) {
            if (typeMap.hasOwnProperty(k)) {
                if (data[k] && isEmpty(data[k])) {
                    data[k] = map[typeMap[k].toLowerCase()];
                }
            }
        }
    }
